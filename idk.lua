...
-- Функция для создания рамок с округлыми углами
...
-- Параметры для создания ключевого окна
local keyFrame = createRoundedFrame(ScreenGui, UDim2.new(0, 400, 0, 230), UDim2.new(0.35, 0, 0.4, 0), Color3.fromRGB(20, 20, 20))
...
local main = createRoundedFrame(ScreenGui, UDim2.new(0, 380, 0, 320), UDim2.new(0.02, 0, 0.6, 0), Color3.fromRGB(35, 35, 35))
main.Visible = false
...
-- Создание кнопок и текстов для вкладок (прикладываю только несколько, для остальных аналогично)
...
createLabel(infoFrame, UDim2.new(1, 0, 0, 30), UDim2.new(0, 0, 0, 0), "Developers: Martusin/Yan, hi1list", 18)
...
-- Открытие меню (кнопка шестерёнки)
local openmain = createRoundedFrame(ScreenGui, UDim2.new(0, 50, 0, 50), UDim2.new(0, 10, 0.8, 0), Color3.fromRGB(50, 50, 50))
...
-- Обработчики для подтверждения ключа
